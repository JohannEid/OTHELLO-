...
bool Board::is_playable(const int &i, const int &j) const {
...
void Board::change_color(int i, int j) {
    std::list<std::pair<int, int>> check_coordinates{std::make_pair(1, 0), std::make_pair(0, 1),
                                                     std::make_pair(-1, 0), std::make_pair(0, -1),
                                                     std::make_pair(1, 1), std::make_pair(-1, -1)};
    std::vector<std::pair<int, int>> temp;
    e_color temp_color = (getNumber_of_turn() % 2 == 0) ? e_color::WHITE : e_color::BLACK;
    e_color opposition_color = (temp_color == e_color::WHITE) ? e_color::BLACK : e_color::WHITE;
    for (const auto &elem: check_coordinates) {
        while (getBoard(i += elem.first, j += elem.second).getColor() == temp_color) {
            temp.push_back(std::make_pair(i, j));
        }
        if (getBoard(i, j).getColor() == temp_color) {
            for (auto &elem_ : temp) {
